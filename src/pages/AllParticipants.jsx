...
function AllParticipantDetails() {
    ...
    const fetchEventsAndParticipants = async () => {
      try {
        ...
        for (const eventDoc of eventsSnapshot.docs) {
          ...
          const participantQuery = query(collection(db, "Registrations"), where("eventid", "==", eventId));
  ...
  return (
    <div className="text-white">
      ...
      {!loading && !error && eventParticipants.map(({ eventName, participants }) => (
              ...
              <tbody className="divide-y divide-gray-200">
                {participants.map((participant, index) => (
                  <React.Fragment key={participant.id}>
                    {index === 0 && (
                      <tr>
                        <th colSpan="2" className="px-6 py-3 text-center text-sm text-white">Participants</th>
                      </tr>
                    )}
                    <tr className="bg-transparent">
                      <td className="px-6 py-4 text-sm text-white">Name: {participant.username}</td>
                      <td className="px-6 py-4 text-sm text-white">Phone: {participant.phone}</td>
                    </tr>
                    <tr className="bg-gray-500">
                      <td className="px-6 py-4 text-sm text-white">NKID: {participant.nkid}</td>
                      <td className="px-6 py-4 text-sm text-white">Email: {participant.email}</td>
                    </tr>
                  </React.Fragment>
                ))}
              </tbody>
